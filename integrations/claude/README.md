...
Este adapter define como o Claude Code consome artefatos do core A8Z.
...
- `command-map.json` é o contrato de mapeamento Claude -> comandos canônicos A8Z.
...
- `.claude/commands/a8z/agents/` contém wrappers adapter para Claude.
- `mcp-specifics.md` contém regras de MCP específicas do Claude em camada de integração.

## Sync de adapters

```bash
node scripts/integrations/sync-claude-agent-adapters.js
node scripts/integrations/sync-claude-agent-adapters.js --check
```
